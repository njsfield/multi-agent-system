...
async function fetchForecast(lat: number, lon: number, timezone: string, dateStr: string): Promise<string> {
...
const fetchWeatherTool = new FunctionTool(
  async (params) => {
    ...
    const offset = dateArg.includes('tomorrow') ? 1
      : dateArg.includes('day after') ? 2
      : 0;
    ...
    try {
      ...
      const raw = await fetchForecast(geo.latitude, geo.longitude, geo.timezone, dateStr);
      ...
      const hourlySlots = h.time
        ...
        .filter(s => ['06:00', '09:00', '12:00', '15:00', '18:00', '21:00'].includes(s.time));
  ...
  'Fetch a live weather forecast from Open-Meteo for any city and date.',
  {
    ...
    properties: {
      ...
      date: { type: 'string', description: '"today", "tomorrow", or "day after tomorrow"' },
    },
...
// ---------------------------------------------------------------------------
// Agent factory — fresh instance per request
// ---------------------------------------------------------------------------
